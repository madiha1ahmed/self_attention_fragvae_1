...
class Trainer:
    ...
    def _train_epoch(self, epoch, loader):
        ...
        epoch_loss_details = np.zeros( (len(loader),8) ) # epoch, idx, loss, CE_loss, MSE_loss, KL_loss, alpha, beta
        self.model.train()
        epoch_loss = 0
        ...
        # what exactly are src, tgt, and lengths
        for idx, (src, tgt, lengths, properties) in enumerate(loader):
            ...
            if self.config.get('use_gpu'):
                src = src.cuda()
                tgt = tgt.cuda()
            output, mu, logvar, predicted_prop = self.model(src, lengths)
            
            if self.config.get('use_gpu'):
                properties = properties.cuda()
                predicted_prop = predicted_prop.cuda()

            loss,CE_loss,MSE_loss,KL_loss,alpha,beta = self.criterion(output, tgt, mu, logvar, 
                                                                      epoch, idx, 
                                                                      properties, predicted_prop)
            loss.backward()
            clip_grad_norm_(self.model.parameters(),
                            self.config.get('clip_norm'))

            epoch_loss += loss.item()
            epoch_loss_details[idx,:]= [epoch, idx, loss.item(), CE_loss, MSE_loss, KL_loss, alpha, beta]

            self.optimizer.step()

        return epoch_loss/len(loader), epoch_loss_details
    ...
    def train(self, loader, start_epoch):
        ...
        for epoch in range(start_epoch, start_epoch + num_epochs):
            ...
            epoch_loss, epoch_loss_details = self._train_epoch(epoch, loader)
