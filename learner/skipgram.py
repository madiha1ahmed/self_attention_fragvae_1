    ...
    def translate(self, values):
        ...
        for v in values:
            if v not in self.TOKEN_IDS:
                # Catch KeyError exception and discard the sample
                try:
                    res.append(self.get(v))
                except:
                    print('Cought KeyError!')
                    continue
            if v == self.EOS:
                break
        return res
...
def train_embeddings(config, data):
    ...
    if use_mask: # use cluster names for low freq words
        ...
        vocab = w2v.wv.key_to_index  # dictionary, word:index
        embeddings = w2v.wv[vocab] # array of size vocab sizex embed_size

        w2f = calculate_frequencies(sentences) # dictionary, w2f['word']: frequency
        w2i = {k: v for (k, v) in vocab.items()}  # dictionary, w2i['word']: index
    ...
    vocab = w2v.wv.key_to_index # vocab with frequent words and cluster names.
    ...
    embeddings = np.vstack([tokens, w2v.wv[vocab]]) # add token embeddings
